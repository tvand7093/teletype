...
suite('TeletypePackage', function () {
  ...
  test('enables button when non-empty token specified', async () => {
    const env = buildAtomEnvironment()
    const pack = await buildPackage(env, {signIn: false})
  ...
  test('shows empty token error if submitted', async () => {
    const env = buildAtomEnvironment()
    const pack = await buildPackage(env, {signIn: false})
    await pack.consumeStatusBar(new FakeStatusBar())
    ...
    assert(description.includes('The specified token most not be empty. Please insert a valid token and try again.'))
  })
